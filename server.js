...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    if (!origin && process.env.NODE_ENV === 'development') {
      return callback(null, true); // Allows localhost in dev mode
    }
    
    // Check if the origin is in the allowed list (for production and other environments)
    if (allowedOrigins.includes(origin)) {
      callback(null, true);
    } else {
      console.log('Blocked CORS request from:', origin);
...
if (process.env.NODE_ENV === 'production') {
  ...
  // Handle routing for frontend (catch-all for React Router)
  app.get('*', (req, res) => {
    res.sendFile(path.join(frontendPath, 'index.html'));
  });
}
